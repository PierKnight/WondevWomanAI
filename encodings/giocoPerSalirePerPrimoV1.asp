...
maxLevelEnemyPawn(X1,Y1,P1,MAXL) :- pawn(X1,Y1,P1,enemy), tile(X1,Y1,MAXL), maxLevelEnemy(MAXL), pawn(X2,Y2,P2,enemy), P1<>P2, tile(X2,Y2,L2), L2<>MAXL.
% 1.2.2. (se entrambe giacciono sullo stesso liv, prendo P1 A PRESCINDERE):
maxLevelEnemyPawn(X1,Y1,P1,MAXL) :- pawn(X1,Y1,P1,enemy), pawn(X2,Y2,P2,enemy), tile(X1,Y1,MAXL), tile(X2,Y2,MAXL), maxLevelEnemy(MAXL).
% 1.3. la pedina nemica P ha il liv MAX_LEVEL_NEIGH come livello max nel suo vicinato
maxLevelVicineNemico(P,MAX_LEVEL_NEIGH) :- maxLevelEnemyPawn(X2,Y2,P,_), #max{L,X1,Y1 : tile(X1,Y1,L), vicine(X1,Y1,X2,Y2)} = MAX_LEVEL_NEIGH.
...
bestVicinaNonMinoreNemico(P,X2,Y2,MAXLEVEL) :- maxLevelEnemyPawn(X1,Y1,P,_), tile(X1,Y1,L1), maxLevelVicineNemico(P,MAXLEVEL), tile(X2,Y2,MAXLEVEL), vicine(X1,Y1,X2,Y2), L1<=MAXLEVEL.
...
numMosseNemicoPerVincere(P,NUMMOSSE) :- maxLevelEnemyPawn(X1,Y1,P,_), tile(X1,Y1,L1), bestVicinaNonMinoreNemico(P,X2,Y2,MAXLEVEL), NUMMOSSE = 6 - L1 - MAXLEVEL.
...
:~ numMosseNemicoPerVincere(ENEMY_PAWN, NUM_MOSSE_NEMICO), nearMoveToPawn(TEAM_PAWN, ENEMY_PAWN, X, Y, DISTANCE), NUM_MOSSE_NEMICO >= DISTANCE - 1,
maxLevelMyPawn(_,_,TEAM_BEST_PAWN,_), numMosseMioPerVincere(TEAM_BEST_PAWN, NUM_MOSSE_MIO), NUM_MOSSE_NEMICO < NUM_MOSSE_MIO, notMoveToCell(X,Y,TEAM_PAWN). [1@8, X,Y]



% 1. Per la mia pedina messa meglio (cioè quella che, delle due, è sul livello maggiore), calcolo il max livello nel proprio vicinato:
% 1.1. il massimo MAXL tra i livelli correnti delle mie pedine:
maxLevelMio(MAXL) :- #max{L,P : pawn(X,Y,P,team), tile(X,Y,L)} = MAXL.
% 1.2.1. la mia pedina P che giace sul livello max MAXL in coordinate X,Y (se le mie due pedine giacciono su due liv differenti):
maxLevelMyPawn(X1,Y1,P1,MAXL) :- pawn(X1,Y1,P1,team), tile(X1,Y1,MAXL), maxLevelMio(MAXL), pawn(X2,Y2,P2,team), P1<>P2, tile(X2,Y2,L2), L2<>MAXL.
% 1.2.2. (se entrambe giacciono sullo stesso liv, prendo P1 A PRESCINDERE):
maxLevelMyPawn(X1,Y1,P1,MAXL) :- pawn(X1,Y1,P1,team), pawn(X2,Y2,P2,team), tile(X1,Y1,MAXL), tile(X2,Y2,MAXL), maxLevelMio(MAXL).
% 1.3. la mia pedina P ha il liv MAX_LEVEL_NEIGH come livello max nel suo vicinato
maxLevelVicineMie(P,MAX_LEVEL_NEIGH) :- maxLevelMyPawn(X2,Y2,P,_), #max{L,X1,Y1 : tile(X1,Y1,L), vicine(X1,Y1,X2,Y2)} = MAX_LEVEL_NEIGH.



% 2. Per la mia pedina migliore (quella con liv corrente maggiore), prendo le coordinate della vicina migliore (con liv maggiore)
% se questa è di livello almeno pari a quello corrente della mia
bestVicinaNonMinoreMia(P,X2,Y2,MAXLEVEL) :- maxLevelMyPawn(X1,Y1,P,_), tile(X1,Y1,L1), maxLevelVicineMie(P,MAXLEVEL), tile(X2,Y2,MAXLEVEL), vicine(X1,Y1,X2,Y2), L1<=MAXLEVEL.

% 3. dopodichè, per la pedina nemica messa meglio (maxLevelEnemyPawn), calcolo quante mosse deve fare
% al minimo per vincere (num mosse minime per vincere = 6 - liv corrente nemico messo meglio - liv best vicina nemico)
numMosseMioPerVincere(P,NUMMOSSE) :- maxLevelMyPawn(X1,Y1,P,_), tile(X1,Y1,L1), bestVicinaNonMinoreMia(P,X2,Y2,MAXLEVEL), NUMMOSSE = 6 - L1 - MAXLEVEL.
