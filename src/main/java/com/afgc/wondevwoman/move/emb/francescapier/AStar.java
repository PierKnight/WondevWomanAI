...
public class AStar {
    ...
    private void setNodes() {
        for (int i = 0; i < searchArea.length; i++) {
            for (int j = 0; j < searchArea[0].length; j++) {
                int level = GameHandler.getInstance().getMyGamePanel().tiles[i][j].getLevel();
                Node node = new Node(i, j, level);
    ...
    private void checkNode(Node currentNode, int col, int row, int cost) {
        ...
        boolean isBlock = adjacentNode.isBlock() || adjacentNode.getLevel() >= 4 ;
        if (!isBlock && !getClosedSet().contains(adjacentNode)) {
